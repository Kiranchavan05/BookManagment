...
const BookDialog = ({ open, handleClose, handleSave, initialBook, loading }) => {
  ...
  useEffect(() => {
    if (initialBook) {
      setBook({
        ...initialBook,
        publication_year: initialBook.publication_year ? String(initialBook.publication_year) : ''
      });
    } else {
  ...
  const isChanged = initialBook
    ? book.title !== initialBook.title ||
      book.author !== initialBook.author ||
      String(book.publication_year) !== String(initialBook.publication_year)
    : true;
    ...
    <Dialog open={open} onClose={handleClose} maxWidth="xs" fullWidth>
      ...
      <form  onSubmit={onSubmit}>
        ...
        <DialogActions>
          ...
          <Button type="submit" variant="contained" sx={{backgroundColor:'black', minWidth: 100}} disabled={loading || !isValid || (initialBook && !isChanged)}>
            {loading ? <CircularProgress size={20} sx={{ color: 'white' }} /> : (initialBook ? 'Update' : 'Add')}
          </Button>
        </DialogActions>
      </form>
    </Dialog>
