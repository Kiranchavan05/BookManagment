...
const API_URL = 'https://book-manager-backend-e8yy.onrender.com/api/books';
...
function AppContent() {
  ...
  const [sortBy, setSortBy] = useState('date'); // Default to 'date' for recently added
  const [order, setOrder] = useState('desc'); // Default to 'desc' for most recent first
  ...
  }, [user, page, rowsPerPage, sortBy, order, search]);
          ...
          <Route
            ...
            element={
              <PrivateRoute user={user}>
                ...
                />
                {/* Delete Dialog */}
                <Dialog open={deleteDialogOpen} onClose={deleteLoading ? undefined : handleDeleteCancel}>
                  ...
                  <DialogActions>
                    <Button onClick={handleDeleteCancel} color="secondary" sx={{ border: '1px solid grey' }} disabled={deleteLoading}>No</Button>
                    <Button onClick={handleDeleteConfirm} variant="contained" sx={{ backgroundColor: 'black' }} disabled={deleteLoading}>
                      {deleteLoading ? <span style={{ display: 'flex', alignItems: 'center' }}><span className="spinner-border spinner-border-sm" role="status" aria-hidden="true" style={{ marginRight: 8 }}></span>Yes...</span> : 'Yes'}
                    </Button>
                  </DialogActions>
                </Dialog>
              </PrivateRoute>
